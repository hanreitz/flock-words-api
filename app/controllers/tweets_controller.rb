class TweetsController < ApplicationController
  ...
  def get_data
    ...
    filtered_words.each do |filtered_word|
      ...
      if existing_word
        word_object = word_object.slice(0,index).push({ text: word_object[index]["text"], value: (word_object[index]["value"] + 1)}).push(word_object.slice(index+1))
      else
        word_object = word_object.push({ text: filtered_word, value: 1 })
      end
    end
